...
function doGet(e) {
  try {
    ...
    if (action === "verifyParticipant") {
      return response(verifyParticipant(String(p.email || ""), String(p.dob || "")));
    }
...
function verifyParticipant(email, dob) {
  email = String(email || "").trim().toLowerCase();
  dob = String(dob || "").trim();
  if (!email || !dob) return { success: false, verified: false, message: "Participant and date of birth are required." };
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const storedDob = normalizeDob(r[2]);
    if (storedDob && storedDob === dob) {
      return { success: true, verified: true, message: "Participant verified." };
    }
    return { success: false, verified: false, message: "Date of birth does not match. Please check and try again." };
  }
  return { success: false, verified: false, message: "Participant not found." };
}
...
function getParticipantStatus(email) {
  ...
  let eligible = false;
  let status = "eligible";
  let statusMessage = "You are eligible for Round 1!";

  if (!completed["Round 1"]) {
    currentRound = "Round 1";
    eligible = true;
    status = "eligible";
    statusMessage = "You are eligible for Round 1!";
  } else if (!completed["Round 2"]) {
    currentRound = "Round 2";
    eligible = eligibleRounds["Round 2"] === true;
    if (eligible) {
      status = "eligible";
      statusMessage = "You qualified for Round 2!";
    } else if (isRoundActivated("Round 2")) {
      status = "not_qualified";
      statusMessage = "You did not qualify for Round 2.";
    } else {
      status = "waiting_for_host";
      statusMessage = "Round 2 is not available yet. Please wait for the host.";
    }
    ...
    if (eligible) {
      status = "eligible";
      statusMessage = "You qualified for Round 3!";
    } else if (isRoundActivated("Round 3")) {
      status = "not_qualified";
      statusMessage = "You did not qualify for Round 3.";
    } else {
      status = "waiting_for_host";
      statusMessage = "Round 3 is not available yet. Please wait for the host.";
    }
    ...
    if (eligible) {
      status = "eligible";
      statusMessage = "You qualified for the Final!";
    } else if (isRoundActivated("Final")) {
      status = "not_qualified";
      statusMessage = "You did not qualify for the Final.";
    } else {
      status = "waiting_for_host";
      statusMessage = "Final is not available yet. Please wait for the host.";
    }
  } else {
    currentRound = "Completed";
    eligible = false;
    status = "completed";
    statusMessage = "You have completed the quiz.";
  }
...
function isRoundActivated(round) {
  ...
  return sh.getRange(2, 1, sh.getLastRow() - 1, 5).getValues().some(function (r) {
    return normalizeRound(r[2]) === target && isTruthy(r[3]);
  });
}
...
function activateRound(payload) {
  ...
  // Results are already sorted by score descending, then fastest time.
  // Admin chooses only the number of qualifiers; the top N are selected automatically.
  const completedRows = resultRows.filter(function (r) {
    return r.round === previousRound;
  }).sort(function (a, b) {
    if (b.score !== a.score) return b.score - a.score;
    return a.durationMs - b.durationMs;
  }).map(function (r, i) {
  ...
  let qualifyCount = Number(payload.qualifyCount);
  if (!isFinite(qualifyCount) || Math.floor(qualifyCount) !== qualifyCount) {
    return fail("Qualification count must be a whole number.");
  }

  if (qualifyCount < 1 || qualifyCount > completedRows.length) {
    return fail("Qualification count must be between 1 and " + completedRows.length + ".");
  }

  const qualifiedRows = completedRows.slice(0, qualifyCount);
  const emails = qualifiedRows.map(function (r) {
    return String(r.email || "").trim().toLowerCase();
  }).filter(Boolean);

  if (!emails.length) return fail("No completed participants are available to qualify.");
  ...
  return {
    success: true,
    message: "Top " + emails.length + " participant(s) qualified for " + nextRound + ".",
    round: nextRound,
    qualifyCount: emails.length,
    participants: emails,
    qualified: qualifiedRows.map(function (r) {
      return { email: r.email, name: r.name, rank: r.rank, score: r.score, timeText: r.timeText };
    })
  };
}
